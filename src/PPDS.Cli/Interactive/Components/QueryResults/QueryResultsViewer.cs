using PPDS.Dataverse.Query;
using Spectre.Console;

namespace PPDS.Cli.Interactive.Components.QueryResults;

/// <summary>
/// Main orchestrator for displaying query results.
/// Chooses between table view and record view based on column count.
/// </summary>
internal static class QueryResultsViewer
{
    /// <summary>
    /// Maximum columns for table view. Above this, record view is used.
    /// </summary>
    public const int TableViewMaxColumns = 6;

    /// <summary>
    /// Result of showing query results.
    /// </summary>
    public enum ShowResult
    {
        /// <summary>User wants to go back to the main menu.</summary>
        Back,
        /// <summary>User wants to enter a new query.</summary>
        NewQuery,
        /// <summary>User wants to exit the entire interactive CLI.</summary>
        Exit
    }

    /// <summary>
    /// Shows query results with automatic view selection.
    /// </summary>
    /// <param name="result">The query result to display.</param>
    /// <param name="fetchPage">Optional function to fetch additional pages.</param>
    /// <param name="environmentUrl">The environment URL for building record links.</param>
    /// <param name="profileName">The profile display name for header context.</param>
    /// <param name="environmentName">The environment display name for header context.</param>
    /// <param name="cancellationToken">Cancellation token.</param>
    /// <returns>The action the user wants to take next.</returns>
    public static async Task<ShowResult> ShowAsync(
        QueryResult result,
        Func<int, string?, Task<QueryResult>>? fetchPage,
        string? environmentUrl,
        string? profileName,
        string? environmentName,
        CancellationToken cancellationToken)
    {
        if (result.Count == 0)
        {
            ShowEmptyResult(result);
            return ShowResult.NewQuery;
        }

        // Special handling for simple aggregate queries (single row with aggregate columns only)
        if (result.IsAggregate && result.Count == 1 && result.Columns.All(c => c.IsAggregate))
        {
            return ShowAggregateResult(result);
        }

        var state = new RecordNavigationState(result, environmentUrl, profileName, environmentName);

        // Choose initial view based on column count
        var useTableView = result.Columns.Count <= TableViewMaxColumns;

        while (!cancellationToken.IsCancellationRequested)
        {
            ViewResult viewResult;

            if (useTableView)
            {
                viewResult = await InteractiveTableView.ShowAsync(state, fetchPage, cancellationToken);
            }
            else
            {
                viewResult = await RecordView.ShowAsync(state, fetchPage, cancellationToken);
            }

            switch (viewResult)
            {
                case ViewResult.Back:
                    return ShowResult.Back;

                case ViewResult.NewQuery:
                    return ShowResult.NewQuery;

                case ViewResult.Exit:
                    return ShowResult.Exit;

                case ViewResult.SwitchToRecordView:
                    useTableView = false;
                    break;

                case ViewResult.SwitchToTableView:
                    useTableView = true;
                    break;
            }
        }

        return ShowResult.Back;
    }

    private static void ShowEmptyResult(QueryResult result)
    {
        AnsiConsole.Clear();

        var content =
            $"{Styles.MutedText("Entity:")} {Markup.Escape(result.EntityLogicalName)}\n" +
            $"{Styles.MutedText("Time:")} {result.ExecutionTimeMs}ms";

        var panel = new Panel(content)
        {
            Border = BoxBorder.Rounded,
            BorderStyle = Styles.HeaderBorder,
            Header = new PanelHeader(" Query Results ", Justify.Center),
            Padding = new Padding(1, 0, 1, 0)
        };

        AnsiConsole.Write(panel);
        AnsiConsole.WriteLine();
        AnsiConsole.MarkupLine(Styles.WarningText("No records found."));
        AnsiConsole.WriteLine();
        AnsiConsole.MarkupLine(Styles.MutedText("Press any key to continue..."));
        Console.ReadKey(true);
    }

    /// <summary>
    /// Shows a special display for simple aggregate queries like COUNT(*).
    /// </summary>
    private static ShowResult ShowAggregateResult(QueryResult result)
    {
        AnsiConsole.Clear();

        var content =
            $"{Styles.MutedText("Entity:")} {Markup.Escape(result.EntityLogicalName)}\n" +
            $"{Styles.MutedText("Time:")} {result.ExecutionTimeMs}ms";

        var panel = new Panel(content)
        {
            Border = BoxBorder.Rounded,
            BorderStyle = Styles.HeaderBorder,
            Header = new PanelHeader(" Aggregate Results ", Justify.Center),
            Padding = new Padding(1, 0, 1, 0)
        };

        AnsiConsole.Write(panel);
        AnsiConsole.WriteLine();

        // Get the single record with aggregate values
        var record = result.Records[0];

        // Create a table for the aggregate results
        var table = new Table
        {
            Border = TableBorder.Rounded,
            BorderStyle = Styles.HeaderBorder
        };

        table.AddColumn(new TableColumn(Styles.MutedText("Aggregate")).Centered());
        table.AddColumn(new TableColumn(Styles.MutedText("Value")).Centered());

        foreach (var column in result.Columns)
        {
            var key = column.Alias ?? column.LogicalName;
            var displayName = column.AggregateFunction?.ToUpperInvariant() ?? "Value";

            if (record.TryGetValue(key, out var value) && value?.Value != null)
            {
                var displayValue = FormatAggregateValue(value, column);
                table.AddRow(
                    Markup.Escape(displayName),
                    $"[bold cyan]{Markup.Escape(displayValue)}[/]"
                );
            }
            else
            {
                table.AddRow(Markup.Escape(displayName), Styles.MutedText("(null)"));
            }
        }

        AnsiConsole.Write(table);
        AnsiConsole.WriteLine();
        AnsiConsole.MarkupLine(Styles.MutedText("[N] New query  [Esc/B] Back  [Q] Exit"));

        // Wait for user input
        while (true)
        {
            var key = Console.ReadKey(true);
            switch (key.Key)
            {
                case ConsoleKey.N:
                    return ShowResult.NewQuery;
                case ConsoleKey.Escape:
                case ConsoleKey.B:
                    return ShowResult.Back;
                case ConsoleKey.Q:
                    return ShowResult.Exit;
            }
        }
    }

    /// <summary>
    /// Formats an aggregate value for display.
    /// </summary>
    private static string FormatAggregateValue(QueryValue value, QueryColumn column)
    {
        // Use formatted value if available
        if (!string.IsNullOrEmpty(value.FormattedValue))
        {
            return value.FormattedValue;
        }

        // Format based on aggregate type
        return column.AggregateFunction?.ToLowerInvariant() switch
        {
            "count" or "countcolumn" => FormatCountValue(value.Value),
            "sum" or "avg" => FormatNumericValue(value.Value),
            _ => value.Value?.ToString() ?? ""
        };
    }

    /// <summary>
    /// Formats a count value with number grouping.
    /// </summary>
    private static string FormatCountValue(object? value)
    {
        if (value is int intVal)
            return intVal.ToString("N0");
        if (value is long longVal)
            return longVal.ToString("N0");
        if (value is decimal decVal)
            return decVal.ToString("N0");
        if (value is double dblVal)
            return dblVal.ToString("N0");
        return value?.ToString() ?? "0";
    }

    /// <summary>
    /// Formats a numeric value (sum, avg) with appropriate precision.
    /// </summary>
    private static string FormatNumericValue(object? value)
    {
        if (value is decimal decVal)
            return decVal.ToString("N2");
        if (value is double dblVal)
            return dblVal.ToString("N2");
        if (value is int intVal)
            return intVal.ToString("N0");
        if (value is long longVal)
            return longVal.ToString("N0");
        return value?.ToString() ?? "0";
    }
}
