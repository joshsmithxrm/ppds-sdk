...
public class ConsoleJsonLoggerProviderTests
{
    [Fact]
    ...
    [Fact]
    public void Logger_UsesCorrelationIdFromContext()
    {
        var options = new CliLoggerOptions { MinimumLevel = LogLevel.Information };
        var context = new LogContext { CorrelationId = "test-correlation-123" };
        using var provider = new ConsoleJsonLoggerProvider(options, context);

        // The correlation ID is embedded in JSON output - we just verify setup doesn't throw
        var logger = provider.CreateLogger("TestCategory");
        Assert.NotNull(logger);
    }

    [Fact]
    public void Logger_UsesCommandNameFromContext()
    {
        var options = new CliLoggerOptions { MinimumLevel = LogLevel.Information };
        var context = new LogContext { CommandName = "data export" };
        using var provider = new ConsoleJsonLoggerProvider(options, context);

        // The command name is embedded in JSON output - we just verify setup doesn't throw
        var logger = provider.CreateLogger("TestCategory");
        Assert.NotNull(logger);
    }
}
